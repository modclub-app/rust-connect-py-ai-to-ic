...
pub async fn create_tensor_and_run_model_pipeline(input: Vec<i64>) -> ModelInferenceResult {
    ...
    for index in 0..num_models as u8 {
        ...
        ic_cdk::println!("Calling model_sub_compute with index: {}, input_shape: {:?}", index, input_shape);
        ...
        match call_result {
            Ok((new_output, new_input_shape)) => {
                ...
                ic_cdk::println!("Call succeeded. new_input_shape: {:?}", new_input_shape);
...
fn model_sub_compute(index: u8, input: TensorInput, input_shape: Vec<usize>) -> (Vec<f32>, Vec<usize>) {
    ...
    ic_cdk::println!("model_sub_compute called with index: {}, input_shape: {:?}", index, input_shape);
